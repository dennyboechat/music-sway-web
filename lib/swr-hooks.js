...
import { songsQuery, songQuery, playlistsQuery, playlistQuery, bandsQuery } from '@/graphQl/queries';
...
export const useUserBands = () => {
  const { data, error } = useSWRImmutable(bandsQuery, query => request('/api/get-user-bands', query, { ownerId: 1 }));
  return {
