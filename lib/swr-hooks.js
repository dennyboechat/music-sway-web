...
import { songsQuery, songQuery, playlistsQuery, playlistQuery } from '@/graphQl/queries';
...
export const usePlaylist = (id) => {
  const { data, error } = useSWR(playlistQuery, query => request('/api/get-playlist', query, { id }));
  return {
